...
class InvertedResidual(nn.Module):
    def __init__(self, input_channels, out_channels, kernel_size=3, stride=1, expand_ratio=4, useChannelAttention=True, reduce_ratio=4):
        super(InvertedResidual, self).__init__()
        assert stride in [1, 2]
        self.stride = stride

        expanded_channels = int(round(input_channels * expand_ratio))
        
        layers: List[nn.Module] = []
        activation_layer = dict(type='ReLU')
        ...
        if expanded_channels != input_channels:
            layers.append(
                ConvModule(
                    ...
                    kernel_size=1,
                    stride=self.stride,
                    conv_cfg=None, # which means using conv2d.
        ...
        if useChannelAttention:
         squeeze_channels = _make_divisible(expanded_channels // reduce_ratio, 8)
...
class BasicBlock(nn.Module):
    ...
    Args:
        ...
        squeeze_excitation (bool): Use channel attention or not.
        expansion_ratios (int): channel expansion ratio.
        reduce_ratios (int): channel reduction ratio.
    ...
    def __init__(self,
        ...
        self.invertRes = nn.ModuleList([
            InvertedResidual(channel, channel, expand_ratio=expansion_ratios, useChannelAttention=squeeze_excitation, reduce_ratio=reduce_ratios)
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            ...
            x2 = [ir(s) for s, ir in zip(x2, self.invertRes)]
...
class LiteHRModule(nn.Module):
    """High-Resolution Module for WideHRNet.
    ...
    def __init__(
        ...
        if self.module_type.upper() == 'LITE':
            self.layers = self._make_weighting_blocks(num_blocks, expansion_ratios, squeeze_excitation, reduce_ratios)
    ...
    def _make_weighting_blocks(self, num_blocks, expansion_ratios, squeeze_excitation, reduce_ratios, stride=1):
...
class WideHRNet(nn.Module):
    ...
    Example:
        >>> from mmpose.models import WideHRNet
        >>> import torch
        >>> extra=dict(
        ...
        >>>        squeeze_excitation=(True, True, True),
        >>>        reduce_ratios=(4, 4, 4),
        >>>        num_channels=(
        >>>            (40, 80),
        >>>            (40, 80, 160),
        ...
        >>>        )),
        >>>    with_head=False)
        >>> self = WideHRNet(extra, in_channels=1)
        >>> self.eval()
    ...
    def _make_stage(self,
        ...
        expansion_ratios = stages_spec['expansion_ratios'][stage_index] 
        squeeze_excitation = stages_spec['squeeze_excitation'][stage_index] 
        reduce_ratios = stages_spec['reduce_ratios'][stage_index] 
